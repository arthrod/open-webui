...
async def get_filtered_models(models, user):
    ...
    """
    Filter models based on a user's read access.
    
    Iterates over the models provided in the "data" field of the input dictionary,
    retrieves detailed information for each model, and returns a list containing only
    the models that the user, identified by their ID, is permitted to access.
    
    Args:
        models (dict): A dictionary expected to contain a "data" key with model entries.
        user: An object with an "id" attribute used for checking access permissions.
    
    Returns:
        list: A list of model dictionaries that the user is authorized to access.
    """
    filtered_models = []
...
async def generate_chat_completion(
    ...
    """
    Generates a chat completion by processing the request and forwarding it to the OpenAI API.
    
    This asynchronous function validates the requested model and checks that the user’s company has sufficient credits to cover the operation. It adjusts the payload with model-specific parameters, handling special cases such as O1 models and streaming responses. For streaming requests, the function accumulates message chunks and records the complete response once the stream ends. In non-streaming cases, it returns the API’s JSON response. The function raises HTTP exceptions for issues such as an unknown model, insufficient credits, or API errors.
      
    Args:
        request: The HTTP request object containing application state information.
        form_data: A dictionary with the chat request payload, including model and message details.
        user: The verified user initiating the request.
        bypass_filter: Optional flag to bypass model access control checks.
    """
    ...
    try:
        ...
        if "text/event-stream" in r.headers.get("Content-Type", ""):
            ...
            async def insert_completion_if_streaming_is_done():
                """
                Processes streaming response chunks and logs a chat completion upon stream termination.
                
                This asynchronous generator iterates over chunks of a streaming HTTP response, decoding and accumulating
                content until a finish signal is encountered. When streaming ends and a valid chat identifier is present,
                the function records the completed chat message along with associated metadata and credit cost. Any JSON
                decoding errors in individual chunks are printed, and each chunk is yielded for real-time processing.
                """
                full_response = ""
...
async def proxy(path: str, request: Request, user=Depends(get_verified_user)):
    """
    Deprecated: Proxies an HTTP request to the configured OpenAI API endpoint.
    
    This asynchronous function forwards the incoming request—including its body and, if enabled,
    user identification headers—to the primary OpenAI API endpoint determined by the configured
    URL and API key. Depending on the response's Content-Type, it returns either a StreamingResponse
    (for server-sent event streams) or a JSON-decoded response. If an error occurs, the function logs
    the exception, extracts relevant error details when possible, and raises an HTTPException with the
