...
def upgrade() -> None:
    # Create the table
    """
    Create the 'model_message_credit_cost' table and insert initial data.
    
    This migration creates the 'model_message_credit_cost' table with two columns:
    model_name, a non-nullable string (max 255 characters) serving as the primary key, and
    message_credit_cost, a non-nullable integer representing the cost for a model's message credit.
    After the table is created, it is populated with predefined rows.
    """
    op.create_table(
...
def downgrade() -> None:
    """
    Drops the model_message_credit_cost table from the database.
    
    This function reverses the changes made in the upgrade migration by removing the
    model_message_credit_cost table.
    """
