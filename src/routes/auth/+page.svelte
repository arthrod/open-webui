		...
		>
			...
			<div class="overflow-y-scroll space-y-4">
				<div>
					...
					pouvoir répondre de manière appropriée et précise à divers prompts, et Lucie se trouve à
					un stade très précoce de son entraînement, n'ayant reçu qu'une seule série de ce que l'on
					appelle l'affinage par instruction. Cet entraînement, bien que léger, permet à Lucie de
					suivre des instructions de base, telles que répondre à une question ou résumer un texte.
					Ses réponses ne sont peut-être pas encore fiables, mais sans cet entraînement
					supplémentaire, Lucie, soumise à une instruction, ne pourrait générer que des chaînes de
					mots sémantiquement liées à cette requête. Ses réponses n'auraient pas la forme correcte.
				</div>
				<div>
					Pour passer aux étapes suivantes de l'entraînement, nous devons collecter de grandes
					quantités de données provenant d'interactions avec les utilisateurs humains. En
					particulier, nous avons besoin de savoir si une réponse fournie par Lucie est bonne,
					mauvaise ou préférable à une autre. Si nous obtenons suffisamment de données de haute
					qualité de ce type, nous pourrons les utiliser pour apprendre à Lucie à aligner ses
					réponses sur les préférences des utilisateurs humains.
				</div>
				<div>
					...
					chaque langue), nous acceptons les interactions dans les deux langues, mais nous portons
					un intérêt particulier à la collecte de données en français, car elles sont beaucoup plus
				...
				<div>
					...
					général d'IA de ce type est tout simplement impossible. Notre intérêt est de créer un
					modèle pour aider à la génération de textes en français et en anglais et pour effectuer
					des tâches pour lesquelles les modèles de langage de base sont performants : résumer des
					documents, répondre à des questions de culture générale (en gardant à l'esprit qu'il n'a
					vu que des données jusqu'en 2023), répondre à des questions sur la base d'un texte
					d'entrée, écrire des histoires, et ainsi de suite. Il n'est pas conçu comme un assistant
					pour les mathématiques ou le codage et il n'est pas capable de fournir des conseils
					éthiques.
				</div>
			</div>
